...
public class JwtTokenProvider {
    ...
    public JwtResponse refreshUserTokens(String refreshToken) throws AccessDediedException {
        JwtResponse jwtResponse = new JwtResponse();
        if (!validateToken(refreshToken)) {
            throw new AccessDediedException();
        }
    ...
    private String getId(String token) {
        return Jwts.parser().setSigningKey(key).build().parseClaimsJws(token).getBody().get("id").toString();
    }
